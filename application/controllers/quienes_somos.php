...
class Quienes_somos extends CI_Controller {
	...
	if ($data_usuario['id_nivel']==1) {
		...
		$crud->display_as('descripcion','Quienes somos');
		...
		$state = $crud->getState();
			if($state == 'add'){
			redirect('quienes_somos/add');
			}
		$this->load->view('../../assets/inc/head_common', $output);
		$this->load->view('../../assets/inc/menu_superior');
		$this->load->view('quienes_somos/quienes_somos',$output);
	...
	public function add(){
		...
		$crud->set_language('spanish');
		$output = $crud->render();
		$consulta=$this->quienes_somos_model->get_quienes_somos();
		if ($consulta) {
			$this->session->set_flashdata('alerta', 'ya guardó una descripcion, si quiere guardar una nueva, borre la anterior y a continuacion cree una nueva');
				redirect('quienes_somos/grilla','refresh');
		}
		$this->load->view('../../assets/inc/head_common_quienes_somos', $output);
			$this->load->view('../../assets/inc/menu_superior');
		$this->load->view('quienes_somos/add',$output );
		$this->load->view('../../assets/inc/footer_common',$output);
	}
	...
	public function guardar_quienes_somos(){
		...
		$descripcion=$this->input->post('txt_descripcion','true');	
			$this->quienes_somos_model->guardar_quienes_somos($descripcion);
