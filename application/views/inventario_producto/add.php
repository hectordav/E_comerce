...
<section id="intro" class="intro-section">
  ...
  <?= validation_errors('<div class="alert alert-danger">','</div> ')?>
             ...
             <label><h3><strong>Agregar Intentario a Producto</strong></h3></label>
             <hr>
        <form  role="form" action="<?php echo $this->config->base_url();?>inventario_producto/guardar_inventario" method="POST"  enctype="multipart/form-data">
            <div class="form-group">
              ...
              <select class="form-control" name="id_producto" id="id_producto" data-show-subtext="true" data-live-search="true"  required="required">
                    <option value="">Seleccione Producto</option>
                     ...
                     foreach ($producto as $i) {
                             echo '<option value="'. $i->id_producto .'"> Codigo '.$i->codigo.' Nombre '.$i->nombre.'</option>';
                            }
                     }else{
                     }
                        ?>
              ...
              <?php endforeach ?>
              <?php endif ?>
               <input type="number" name="txt_cantidad" id="txt_cantidad" class="form-control" value="<?= set_value("txt_cantidad")?>" placeholder="Ingrese cantidad">
            </div>
            <div class="form-group">
              <label>Precio Neto</label>
               <input type="text" name="txt_precio_neto" id="txt_precio_neto" onkeyup="fncSumar()" class="form-control" value="<?= set_value("txt_precio_neto")?>" placeholder="Ingrese Precio">
              </select>
            </div>
             <div class="form-group">
              <label>Iva</label>
              <input type="text" name="txt_iva_2" id="txt_iva_2" class="form-control" value="<?= set_value("txt_iva_2")?>" placeholder="" readonly>
            </div>
             <div class="form-group">
              <label>Total</label>
              <input type="text" name="txt_total" id="txt_total" class="form-control" value="<?= set_value("txt_total")?>" placeholder="" readonly>
            </div>
